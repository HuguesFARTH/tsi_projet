...
class Object3D(Object):
    ...
    def render(self):
        ...
        # Fog
        loc = GL.glGetUniformLocation(self.program, "skyColor")
        # Vérifie que la variable existe
        if (loc == -1) :
            print("Pas de variable uniforme : skyColor")
...
class Camera3P(Camera):
    ...
    def update(self):
        ...
        if roll == math.pi/2:
            yaw = self.transformation.rotation_euler[pyrr.euler.index().yaw]
        else:
            if dir[0] < 0:
                # print("1")
                yaw = math.pi - math.acos(dir[2])#*math.cos(roll)
            else:
                # print("2")
                yaw = math.acos(dir[2]) + math.pi#*math.cos(roll)#
